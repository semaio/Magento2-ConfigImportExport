...
$ php bin/magento config:data:import --help
 ...
 Options:
  ...
  --lock (-l)                      Additionally lock imported values in app/etc/config.php (read-only in Admin).
```
...
```bash
php bin/magento config:data:import config/store production --lock
```
...
:exclamation: When using `--lock` together with `!!DELETE`, the value is removed from the database but **not** from `config.php`. If the value was previously locked, you need to remove it from `app/etc/config.php` manually.

:exclamation: The `--lock` option writes to the filesystem. Make sure `app/etc/config.php` is writable by the PHP process.
